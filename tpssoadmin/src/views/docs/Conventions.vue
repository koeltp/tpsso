...
外部 NuGet：
  ...
  （被 Application、Infrastructure、Auth、Admin 引用，Domain 不引用）</pre>
    </el-card>
    <p>依赖倒置：Application 定义接口，Infrastructure 实现接口，Auth/Admin 作为组合根负责 DI 装配。依赖方向只能从外向内，内层不知道外层的存在。Domain 层零外部依赖，不引用 TaiPi.Core。</p>
